...
function updateGoalStats() {
  ...
  //loop through activities to calculate totals
  activities.forEach((activity) => {
    totalCalories += activity.calories_burned_per_hour;
    totalDuration += activity.duration_minutes;
    totalBurnedCalories +=
      (activity.calories_burned_per_hour / 60) * activity.duration_minutes;
  });
  ...
  document.getElementById("total-burned").textContent =
    totalBurnedCalories.toFixed(2) + " cal";
}
...
function generateGoalsChart() {
  ...
  if (!activities || activities.length === 0) {
    ...
    document
      .getElementById("goalsChart")
      .getContext("2d")
      .clearRect(0, 0, 400, 400); // Clear the canvas
    return;
  }

  const activityNames = activities.map((activity) => activity.name);
  const caloriesBurned = activities.map(
    (activity) => activity.calories_burned_per_hour
  );
